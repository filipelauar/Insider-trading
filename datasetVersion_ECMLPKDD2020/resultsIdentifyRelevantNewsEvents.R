...
#Filter days with return smaller then 0.05 percentile and bigger the 0.95 percentile
data = subset(data ,data$return <= 0.05 | data$return >= 0.95)

#Split data respecting temporality
train = subset(data, data$date > '2017-01-30' & data$date < '2017-07-11')
valid = subset(data, data$date > '2017-07-10' & data$date < '2017-12-29')
test = subset(data, data$date > '2017-12-29')


train.y = train$scoreNewsEvent
valid.y = valid$scoreNewsEvent
test.y = test$scoreNewsEvent
...
resultValid = recall90(predValidProbability,dvalid)
